...
void cleanWindow(window* w)
{
    ...
    window_element* cur = w->head;
        ...
        if (cur != NULL && cur->status != WE_ACK && cur->status != WE_NOT_SENT) {
...
int isCorrupt(struct window_element* we)
{
    int a = computeCheckSum(we->packet->buffer + HEADER_SIZE, we->packet->total_size);
    int b = we->packet->checkSum;
    return a != b;
}
